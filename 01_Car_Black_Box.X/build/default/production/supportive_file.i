...
unsigned char clcd_screen_char[15], eeprom_address = 0x00, log_data[10][15];
...
void view_dashboard(unsigned char detected_key) {
    ...
    clcd_print((const unsigned char *) "   Time   EV SP ", (0x80 + (0)));
    clcd_print(clcd_screen_char, (0xC0 + (1)));
}

void display_main_menu(unsigned char detected_key) {

    if (detected_key == 1 || (menu_flag == 2 && detected_key != 2)) {

        if (detected_key == 1) clcd_write(0x01, 0);
        ...
        if (menu_count) clcd_print((const unsigned char*) "->", (0xC0 + (0)));
        else clcd_print((const unsigned char*) "->", (0x80 + (0)));

        clcd_print(menu_list[menu_count - arrow_count], (0x80 + (3)));
        clcd_print(menu_list[menu_count - arrow_count + 1], (0xC0 + (3)));
...
void view_log(unsigned char detected_key) {
    ...
    } else if (detected_key == 12) {
        if (--count_view <= 0) count_view = 0;
    }

    clcd_print((const unsigned char *) "L   Time   EV SP", (0x80 + (0)));
    clcd_putch((const unsigned char) ('0' + (unsigned int) count_view), (0xC0 + (0)));
    clcd_print(log_data[count_view], (0xC0 + (2)));
...
void set_time(unsigned char detected_key) {
    ...
    clcd_print((const unsigned char*) "Set Time", (0x80 + (4)));

    if (detected_key == 12 && ++time_fild == 3) time_fild = 0;
    else if (detected_key == 11 && time_fild == 0 && ++hur_count >= 24) hur_count = 0;
    else if (detected_key == 11 && time_fild == 1 && ++min_count >= 60) min_count = 0;
    else if (detected_key == 11 && time_fild == 2 && ++sec_count >= 60) sec_count = 0;
    else {
        switch (time_fild) {
            case 0:
                if (delay++ < 500) clcd_print((const unsigned char*) "  ", (0xC0 + (4)));
                else if (delay < 1000) clcd_print(time, (0xC0 + (4)));
                else delay = 0;
                break;
            case 1:
                if (delay++ < 500) clcd_print((const unsigned char*) "  ", (0xC0 + (7)));
                else if (delay < 1000) clcd_print(time, (0xC0 + (4)));
                else delay = 0;
                break;
            case 2:
                if (delay++ < 500) clcd_print((const unsigned char*) "  ", (0xC0 + (10)));
                else if (delay < 1000) clcd_print(time, (0xC0 + (4)));
                else delay = 0;
                break;
        }
    ...
    if (detected_key == 1) {
        ...
        write_ds1307(0x02, ((unsigned char) ((hur_count / 10) << 4) | (hur_count % 10)));
        write_ds1307(0x01, ((unsigned char) ((min_count / 10) << 4) | (min_count % 10)));
        write_ds1307(0x00, ((unsigned char) ((sec_count / 10) << 4) | (sec_count % 10)));
...
void download_log(void) {
    ...
    clcd_print((const unsigned char*) "Log download", (0x80 + (2)));
    clcd_print((const unsigned char*) "View in Teraterm", (0xC0 + (0)));
...
void clear_log(void) {
    ...
    clcd_print((const unsigned char *) "Clear Logs   ", (0x80 + (3)));
    clcd_print((const unsigned char *) "Clear Successful", (0xC0 + (0)));
...
void event_store(void) {
    ...
    for (i = 0; i < 0x8B; i++)
        write_external_eeprom(i, read_external_eeprom(i + 14));
    ...
    strcpy((char *) log_data[10 - 1], (const char *) clcd_screen_char);
